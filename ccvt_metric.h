...
namespace ccvt {
  ...
  struct MetricEuclidean {
    ...
    inline Point centroid(const Point&, const vector<Point>& points) const {
      Point centroid;
      int pointsSize = static_cast<int>(points.size());
      for (int j = 0; j < pointsSize; ++j) {
        for (int i = 0; i < 3; ++i) {
  ...
  struct MetricAniso2d {
	...
	inline double distance(const AnisoPoint2f& p1, const AnisoPoint2f& p2) const {
      return sqrt(distance_square(p1, p2));
    }
    ...
    inline AnisoPoint2f centroid(const AnisoPoint2f&, const vector<AnisoPoint2f>& points) const {
